...
double RMSD(double * coords1, double * coords2, int n, double R[3][3])
{
    ...
    if (eigenval[0] >= eigenval[1])
    {
        ...
        else // eigenval[1] <= min{ eigenval[0], eigenval[2] }
        {
    ...
    else // eigenval[1] > eigenval[0]
    {
        if (eigenval[2] >= eigenval[1])
        {
            // eigenval[2] >= eigenval[1] > eigenval[0]
            eigenval0 = eigenval[2];
            for (int i=0; i < 3; i++) eigenvec0[i] = eigenvec[i][2];
            eigenval1 = eigenval[1];
            for (int i=0; i < 3; i++) eigenvec1[i] = eigenvec[i][1];
        ...
        else // eigenval[1] >= max{ eigenval[0], eigenval[2] }
        {
...
double fast_rmsd(double * coords1, double * coords2, int n)
{
    ...
    double a1 = e01 + e02 + e01*e02 - e11*e11 - e22*e22 - e12*e12;
    double a0 = e11*e11*e02 + e12*e12 + e22*e22*e01 - e01*e02 - 2*e11*e22*e12;
    double roots[3];

    cubic_roots(a0, a1, a2, roots);
    roots[0] *= x;
    roots[1] *= x;
    roots[2] *= x;
    ...
    else // roots[2] is still smallest
    {
        if (roots[1] < roots[2])
            min_index = 1;
    }
